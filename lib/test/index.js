...
ruleTester.run('no-moment-methods/seconds', rule[ "no-moment-methods" ].seconds, {
  valid: [],
  invalid: [
    {
      code: "moment().seconds()",
      errors: [ {
        message: 'Consider using new Date().getSeconds() or new Date().setSeconds()',
        type: 'CallExpression'
      } ]
    }
  ]
});
ruleTester.run('no-moment-methods/is-after', rule[ "no-moment-methods" ][ "is-after" ], {
  valid: [],
  invalid: [
    {
